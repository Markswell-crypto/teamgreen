...
import Logo from '../images/linda.png';
...
const Navbar = ({ isLoggedIn }) => {
  ...
  const menuitems = [
    ...
    { path: '/signup', link: 'Signup', hideWhenLoggedIn: isLoggedIn },
    { path: '/login', link: 'Login', hideWhenLoggedIn: isLoggedIn },
    { path: '/appointment-history', link: 'Appointment History', showWhenLoggedIn: isLoggedIn },
    { path: '/book-appointment', link: 'Book Appointment', showWhenLoggedIn: isLoggedIn },
  ];
    ...
    <div className={`md:px-16 fixed bg-white bg-opacity-95 z-100 top-0 right-0 w-full px-4 flex justify-between items-center transition-all duration-300 ${showShadow ? 'shadow-md' : ''}`}>
      ...
      <div className='w-1/2 hidden md:flex items-center gap-8 text-lg font-medium '>
        {menuitems.map((item, index) => (
          (!item.hideWhenLoggedIn || !isLoggedIn) && (!item.showWhenLoggedIn || isLoggedIn) &&
      ...
      <div className=' hidden md:flex justify-end w-1/4'>
        {isLoggedIn ?
          <Link to='/logout' className='bg-black text-white text-lg font-medium px-8 py-4 rounded-full'>Logout</Link> :
          <Link to='/login' className='bg-black text-white text-lg font-medium px-8 py-4 rounded-full'>Login</Link>}
      </div>
